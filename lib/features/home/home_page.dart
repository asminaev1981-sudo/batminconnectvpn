...
class HomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final connecting = state.status == TunnelStatus.connecting ||
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          SegmentedButton<VpnProtocol>(
                            segments: const [
                              ButtonSegment(
                                value: VpnProtocol.hysteria2,
                                label: Text('Hysteria2'),
                                icon: Icon(Icons.bolt),
                              ),
                              ButtonSegment(
                                value: VpnProtocol.amneziaWg,
                                label: Text('AmneziaWG'),
                                icon: Icon(Icons.shield_outlined),
                              ),
                              ButtonSegment(
                                value: VpnProtocol.auto,
                                label: Text('AUTO'),
                                icon: Icon(Icons.auto_awesome),
                              ),
                            ],
                            selected: {selectedProtocol},
                            onSelectionChanged: busy || connected
                                ? null
                                : (selection) {
                                    controller.selectProtocol(selection.first);
                                  },
                          ),
                          const SizedBox(height: 10),
                          Text(
                            selectedProtocol == VpnProtocol.auto
                                ? 'AUTO пока не активирован — сначала проверяем Hysteria2 и AmneziaWG.'
                                : 'Выбран: ${selectedProtocol.title}',
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _Metric(
                          label: 'PING',
                          value: state.pingMs == 0 ? '—' : '${state.pingMs} ms',
                        ),
                      ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      address,
                      style: const TextStyle(color: Color(0xFF7E8AA6)),
                    ),
                  ],
                ),
              ),
